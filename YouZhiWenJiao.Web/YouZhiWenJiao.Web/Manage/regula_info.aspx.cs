...
{
    public partial class regula_info : CommonPage
    {
        int IntID = 0;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            href_value = href_string + "id=" + IntID + "03";

            if (!IsPostBack)
            {
                if (IntID > 0)
                {
                    sqlCmd.CommandText = @"select content,title,Datetime from Regulation where Regulation.id=@id";
        ...
        protected void btnOK_Click(object sender, System.EventArgs e)
        {
            if (IntID > 0)
            {
                sqlCmd.CommandText = "update Regulation set content=@content,Datetime=@date,title=@title where id=@id";
            }
            else
            {
                sqlCmd.CommandText = "insert into Regulation (content,Datetime,title)values(@content,@date,@title)";
            }
