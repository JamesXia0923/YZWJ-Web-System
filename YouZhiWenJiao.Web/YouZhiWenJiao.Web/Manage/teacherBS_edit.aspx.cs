...
{
	public partial class teacherBS_edit : CommonPage
	{
		protected string productId = "";

		string user = @"";
		string imgUrl = @"";
		string href_string = @"../teacherBS.aspx?";
...
where categoryid = @categotyid";
                ...
                sqlCmd.CommandText = sqlCmd.CommandText.Replace("@categotyid", "'" + ((int)category.教学软件).ToString() + "'");
...
1);";
            ...
            sqlCmd.CommandText = sqlCmd.CommandText.Replace("@categoryid", "'" + ((int)category.教学软件).ToString() + "'");
			...
			href_value = href_string + "id=" + productId;
			Alert("保存成功!");
			Response.Redirect("teacherBS.aspx", false);
		}
